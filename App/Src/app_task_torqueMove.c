...
void StartTorqueMove(void *argument)
{
    ...
    int32_t current_l = 0, current_r = 0; //左右力矩参数
    int32_t filter_l_buf[FILTER_NUM] = {0}, filter_r_buf[FILTER_NUM] = {0}; //滤波缓存
    ...
    for(;;)
    {
        ...
        if (safe_motor_enable) // 台车使能按钮按下 key3
        { 
            osThreadFlagsClear(FLAG_U2_ACK_READY | FLAG_U4_ACK_READY);
            osThreadFlagsClear(FLAG_TORQUE_READY);  //清除力矩就绪标志，准备接收新数据
            BSP_Torque_RequestData(TORQUE_MODE_DOUBLE);  //发送里力矩请求
           //运行一次开启功能
            if (!servo_is_enabled) {
            ...
            //等待力矩数据解析完成的标志位
            uint32_t wait_flags = osThreadFlagsWait(FLAG_TORQUE_READY, osFlagsWaitAny, 15);//15ms之内需要回馈
